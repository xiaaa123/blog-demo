...
function loadWidget(waifuPath, apiPath) {
    ...
    $("body").append(`<div id="waifu">
			...
			<canvas id="live2d" width="300" height="300"></canvas>
			   <div class="live2d-tool switch-live2d no-select"><div class="keys">Switch</div></div>
			   <div class="live2d-tool skin-live2d no-select"><div class="keys">Skin</div></div>
			   <div class="live2d-tool save-live2d no-select"><div class="keys">Save</div></div>
 			   <div class="live2d-tool hide-live2d no-select"><div class="keys">Hide</div></div>
		</div>`);
    $("#waifu").show().animate({
        bottom: -12
    }, 3000);
    ...
    function registerEventListener() {
        $(".switch-live2d").click(loadOtherModel);
        $(".skin-live2d").click(loadRandModel);
        $(".save-live2d").click(() => {
        ...
        $(document).on("visibilitychange", () => {
            if (!document.hidden) showMessage("哇，你终于回来了～", 3000, 9);
        });
    }
    ...
    function welcomeMessage() {
        ...
        if (location.href == SiteIndexUrl) { //如果是主页
            ...
            if (location.hostname == referrer.hostname) text = `欢迎阅读<span style="color:orange;">『${document.title.split(" | ")[0]}』</span>`;
            else if (domain == "baidu") text = `Hello！来自 百度搜索 的朋友<br>你是搜索 <span style="color:orange;">${referrer.search.split("&wd=")[1].split("&")[0]}</span> 找到的我吗？`;
            else if (domain == "so") text = `Hello！来自 360搜索 的朋友<br>你是搜索 <span style="color:orange;">${referrer.search.split("&q=")[1].split("&")[0]}</span> 找到的我吗？`;
            else if (domain == "google") text = `Hello！来自 谷歌搜索 的朋友<br>欢迎阅读<span style="color:orange;">『${document.title.split(" - ")[0]}』</span>`;
            else text = `Hello！来自 <span style="color:orange;">${referrer.hostname}</span> 的朋友`;
        } else {
            text = `欢迎阅读<span style="color:orange;">『${document.title.split(" | ")[0]}』</span>`;
        }
    ...
    function showHitokoto() {
        ...
        if (Math.random() < 0.6 && messageArray.length > 0) showMessage(messageArray[Math.floor(Math.random() * messageArray.length)], 6000, 9);
            ...
            var text = `这句一言来自 <span style="color:orange;">『${result.from}』</span>，是 <span style="color:orange;">${result.creator}</span> 在 hitokoto.cn 投稿的。`;
    ...
    function loadRandModel() {
        ...
        $.ajax({
            cache: false,
            url: `${apiPath}/switch_textures/?id=${modelId}-${modelTexturesId}`,
            dataType: "json",
...
function initWidget(waifuPath = "/waifu-tips.json", apiPath = "") {
    ...
    if (localStorage.getItem("waifu-display") && new Date().getTime() - localStorage.getItem("waifu-display") <= 86400000) {
        $("#waifu-toggle").attr("first-time", true).css({
            "margin-left": -50
        });
    } else {
        loadWidget(waifuPath, apiPath);
